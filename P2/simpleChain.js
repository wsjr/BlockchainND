...
class Blockchain{
  ...
  getBlockHeight(){
    ...
    return new Promise((resolve, reject) => {
      self.db.createReadStream({ keys: true, values: false }).on('data', function(data) {
          i++;
      }).on('error', function(err) {
...
(function theLoop (i) {
    setTimeout(function () {
        ...
        myBlockChain.addBlock(blockTest).then((result) => {
          console.log("Adding 'Test Block - " + (i + 1) + "', result is: " + result);
          i++;
          if (i < BLOCKS_TO_ADD) theLoop(i);

          // Validate the whole chain after all the blocks have been added.
          if (i === BLOCKS_TO_ADD) {
            // Delay it a bit for formatting purposes.
            setTimeout(function() {
              console.log("=== Blocks in the chain ===");
